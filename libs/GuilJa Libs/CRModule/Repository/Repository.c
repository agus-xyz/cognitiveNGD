...
void CRM_Repo_NodoPropio(REPO_MSSG_RCVD *Peticion)
{
    ...
    switch (*((BYTE*)(Peticion->Param1)))
    {
        case AllOwnNode:
            ...
            InfoPropioNodo.Channel = currentChannel;
            //InfoPropioNodo.Address = myLongAddress;
            for (i = 0; i < MY_ADDRESS_LENGTH; i++)
            {
                InfoPropioNodo.Address[i] = myLongAddress[i];
            }
            InfoPropioNodo.PANID = myPANID;
...
BOOL CRM_Repo_NodosRed(REPO_MSSG_RCVD *Peticion)
{
    ...
    {
        if (Misma_EUI(Peticion->EUINodo, ConnectionTable[i].Address))
        {
