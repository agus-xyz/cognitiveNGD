 ...
 *           WFSetFuncState must be called when entering and leaving this 
 *           function. When WF_ASSERT is enabled this allows a runtime check if
 *           any illegal WF functions are called from within this function.
...
{
	#if defined (STACK_USE_UART ) || defined (WIFI_USB_DBG)//DIA 7_7_11
		char buf[8];
		char USB_Out_Buffer[64];
	#endif	

    #if defined(STACK_USE_UART)
    //char buf[8];//DIA 7_7_11
    ...
    putsUART(buf);
    #endif
	#if defined(WIFI_USB_DBG)//DIA 7_7_11

		copyArrayToUSBBuffer("WF ASSERTION: Module Number = ", sizeof("WF ASSERTION: Module Number = "));
		sprintf(buf, "%d  ", moduleNumber);
		copyArrayToUSBBuffer(buf, strlen(buf));
