...
#include "HardwareProfile.h"
    ...
    #error "Error in HardwareProfile.h. Must select either the ENC28J60 or the "
           "MRF24WB10 but not both ENC_CS_TRIS and WF_CS_TRIS."
#endif
#if defined(ENC100_INTERFACE_MODE) && defined(WF_CS_TRIS)
    #error "Error in HardwareProfile.h. Must select either the ENCX24J600 or "
           "the MRF24WB10 but not both ENC100_INTERFACE_MODE and WF_CS_TRIS."
#endif
#if defined(ENC100_INTERFACE_MODE) && defined(ENC_CS_TRIS)
    #error "Error in HardwareProfile.h. Must select either the ENC28J60 or the "
           "ENCX24J600 but not both ENC_CS_TRIS and ENC100_INTERFACE_MODE."
#endif

#if !defined(ENC_CS_TRIS) && !defined(WF_CS_TRIS) && !defined(ENC100_INTERFACE_MODE) && \
    (defined(__18F97J60)  || defined(__18F96J65)  || defined(__18F96J60) || \
...
#else
    #error No Ethernet/WiFi controller defined in HardwareProfile.h. Defines for
           an ENC28J60, ENC424J600/624J600, or WiFi MRF24WB10 must be present.
#endif
...
// MAC RAM definitions
    ...
    #define RXSTART     ((TXSTART + 1518ul + TCP_ETH_RAM_SIZE + \
                         RESERVED_HTTP_MEMORY + RESERVED_SSL_MEMORY + \
                         RESERVED_CRYPTO_MEMORY + 1ul) & 0xFFFE)
    ...
    #define TXSTART             (RAMSIZE - ((4ul + MAX_PACKET_SIZE + 4ul)*2) - \
                                 TCP_ETH_RAM_SIZE - RESERVED_HTTP_MEMORY - \
                                 RESERVED_SSL_MEMORY)
    #define RXSTART             (0ul)
    ...
    #error Warning, Ethernet RX buffer is tiny. Reduce TCP socket count, the
           size of each TCP socket, or move sockets to a different RAM
#endif
 ...
 * Output:      None
 * Side Effects:None
 * Overview:    Writes the value of NewConfig into the PHLCON PHY register. The
 *              LED pins will beginning outputting the new configuration
 *              immediately.
 * Note:        None
 ******************************************************************************/
