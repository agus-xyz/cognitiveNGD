...
/*******************************************************************************
 * Memory Configuration: The following section sets up the memory types for use
 * by this application.
 ******************************************************************************/
...
/*******************************************************************************
 * User Configuration: Load the user-specific configuration from TCPIPConfig.h
 ******************************************************************************/
#include "TCPIPConfig MRF24WB0M.h"
// Juan: Config file directly chosen, instead of inclulding TCPIPConfig and
// defining a macro with the configuration file.

/*******************************************************************************
 * Configuration Rules Enforcement: The following section enforces requirements
 * for modules based on configurations selected in TCPIPConfig MRF24WB0M.h
 ******************************************************************************/
...
// memory (instead of external EEPROM).
      ...
      defined(STACK_USE_MPFS2)) && defined(STACK_USE_FTP)
...
// Make sure that the DNS client is enabled if services require it
    ...
    defined(STACK_USE_SNTP_CLIENT) || defined(STACK_USE_DYNAMICDNS_CLIENT) || \
...
// depends on it
#if defined(STACK_USE_FTP_SERVER) || defined(STACK_USE_SNMP_SERVER) || \
    defined(STACK_USE_DNS) || defined(STACK_USE_GENERIC_TCP_CLIENT_EXAMPLE) || \
    defined(STACK_USE_TFTP_CLIENT) || defined(STACK_USE_SMTP_CLIENT) || \
    defined(STACK_USE_ICMP_CLIENT) || defined(STACK_USE_DYNAMICDNS_CLIENT) || \
    defined(STACK_USE_SNTP_CLIENT) || defined(STACK_USE_BERKELEY_API) || \
    defined(STACK_USE_SSL_CLIENT)  || defined(STACK_USE_AUTO_IP)
...
// depends on it
#if defined(STACK_USE_UART2TCP_BRIDGE) || defined(STACK_USE_HTTP2_SERVER)  || \
    defined(STACK_USE_FTP_SERVER)      || defined(STACK_USE_TELNET_SERVER) || \
    defined(STACK_USE_GENERIC_TCP_CLIENT_EXAMPLE) || \
    defined(STACK_USE_GENERIC_TCP_SERVER_EXAMPLE) || \
    defined(STACK_USE_SMTP_CLIENT) || defined(STACK_USE_TCP_PERFORMANCE_TEST) || \
    defined(STACK_USE_DYNAMICDNS_CLIENT) || defined(STACK_USE_BERKELEY_API)   || \
    defined(STACK_USE_SSL_CLIENT) || defined(STACK_USE_SSL_SERVER)
...
// Make sure that STACK_USE_UDP is defined if a service depends on it
#if defined(STACK_USE_DHCP_CLIENT) || defined(STACK_USE_DHCP_SERVER)          || \
    defined(STACK_USE_DNS)         || defined(STACK_USE_NBNS)                 || \
    defined(STACK_USE_SNMP_SERVER) || defined(STACK_USE_TFTP_CLIENT)          || \
    defined(STACK_USE_ANNOUNCE)    || defined(STACK_USE_UDP_PERFORMANCE_TEST) || \
    defined(STACK_USE_SNTP_CLIENT) || defined(STACK_USE_BERKELEY_API)
...
#if defined(STACK_USE_MPFS2) && defined(MPFS_USE_SPI_FLASH) && \
    ((MPFS_RESERVE_BLOCK & 0x0fff) != 0)
...
#include "Common/TimeDelay.h"   //Instead of "TCPIP Stack/Delay.h". Conflicting functions
