/*******************************************************************************
 *	Microchip LSI TCP/IP Stack Application Configuration Header
 *******************************************************************************
 * FileName:        TCPIPConfig MRF24WB0M.h
 ...
 *      signal controller product ("Device") which is integrated into Licensee's
 *      product; or
 * (ii) ONLY the Software driver source files ENC28J60.c, ENC28J60.h, 
 *      ENCX24J600.c and ENCX24J600.h ported to a non-Microchip device used in
 *      conjunction with a Microchip ethernet controller for the sole purpose of
 *      interfacing with the ethernet controller.
 *
...
//   Application Options
// =============================================================================
/* Application Level Module Selection: Uncomment or comment the following lines
 * to enable or disabled the following high-level application modules.        */

//#define STACK_USE_UART		// Application using UART for IP address display and stack config
...
/* Module for testing UDP TX performance characteristics. NOTE: Enabling this
 * will cause a huge amount of UDP broadcast packets to flood your network on
 * the discard port. Use care when enabling this on production networks,
 * especially with VPNs (could tunnel broadcast traffic across a limited
...
// =============================================================================
/* MPFS Configuration: MPFS is automatically included when required for other
 * applications. If your custom application requires it otherwise, uncomment the
 * appropriate selection.                                                     */
//#define STACK_USE_MPFS
//#define STACK_USE_MPFS2

/* MPFS Storage Location: If html pages are stored in internal program memory,
 * comment both MPFS_USE_EEPROM and MPFS_USE_SPI_FLASH, then include an MPFS
 * image (.c or .s file) in the project. If html pages are stored in external
 * memory, uncomment the appropriate definition.
 * Supported serial flash parts include the SST25VFxxxB series.               */
//#define MPFS_USE_EEPROM
//#define MPFS_USE_SPI_FLASH

/* EEPROM Addressing Selection: If using the 1Mbit EEPROM, uncomment this line*/
//#define USE_EEPROM_25LC1024

/* EEPROM Reserved Area: Number of EEPROM bytes to be reserved before MPFS
 * storage starts. These bytes host application configurations such as IP
 * Address, MAC Address, and any other required variables.
 * For MPFS Classic, this setting must match the Reserved setting on the
 * Advanced Settings page of the MPFS2 Utility.                               */
#define MPFS_RESERVE_BLOCK				(205ul)

/* MPFS File Handles: Maximum number of simultaneously open MPFS2 files.
...
// =============================================================================
/* Default Network Configuration: These settings are only used if data is not
 * found in EEPROM.                                                           */
#define MY_DEFAULT_HOST_NAME		"CWSN_LSI_Node"

/* Use the default of 00-04-A3-00-00-00 if using an ENCX24J600, MRF24WB0M, or
 * PIC32MX6XX/7XX internal Ethernet controller and wish to use the internal
 * factory programmed MAC address instead.                                    */
 ...
 * otherwise the default connection will be attempted depending on the selected
 * PHY                                                                        */
...
#define EMAC_TX_DESCRIPTORS	2   // Number of TX descriptors to be created
#define EMAC_RX_DESCRIPTORS	8   // Number of RX descriptors and RX buffers
                                    // to be created

#define	EMAC_RX_BUFF_SIZE	1536
/* Size of a RX buffer. should be multiple of 16. This is the size of all
 * receive buffers processed by the ETHC. The size should be enough to
 * accomodate any network received packet. If the packets are larger, they will
 * have to take multiple RX buffers. The current implementation does not handle
 * this situation right now and the packet is discarded.                      */

// =============================================================================
//   Transport Layer Options
// =============================================================================

/* Transport Layer Configuration: The following low level modules are
 * automatically enabled based on module selections above. If your custom module
 * requires them otherwise, enable them here.                                 */
#define STACK_USE_TCP
#define STACK_USE_UDP

/* Client Mode Configuration: Uncomment following line if this stack will be
 * used in CLIENT mode. In CLIENT mode, some functions specific to client
 * operation are enabled.                                                     */
#define STACK_CLIENT_MODE

/* TCP Socket Memory Allocation: TCP needs memory to buffer incoming and
 * outgoing data. The amount and medium of storage can be allocated on a
 * per-socket basis using the example below as a guide.
 * Allocate how much total RAM (in bytes) you want to allocate for use by your
 * TCP TCBs, RX FIFOs, and TX FIFOs.                                          */
    ...
    #if defined(__TCP_C)
        // Define what types of sockets are needed, how many of each to include,
        // where their TCB, TX FIFO, and RX FIFO should be stored, and how big
        // the RX and TX FIFOs should be. Making this initializer bigger or
        // smaller defines how many total TCP sockets are available.
        // Each socket requires up to 56 bytes of PIC RAM and
        // 48+(TX FIFO size)+(RX FIFO size) bytes of TCP_*_RAM each.
        // Note: The RX FIFO must be at least 1 byte in order to receive SYN and
        // FIN messages required by TCP. The TX FIFO can be zero if desired.
...
/* UDP Socket Configuration: Define the maximum number of available UDP Sockets,
 * and whether or not to include a checksum on packets being transmitted.     */
 ...
 * or PIC32MX6XX/7XX, which have a super fast DMA and incurs virtually no speed
 * pentalty.                                                                  */

/* Berkeley API Sockets Configuration: Note that each Berkeley socket internally
 * uses one TCP or UDP socket defined by MAX_UDP_SOCKETS and the
 * TCPSocketInitializer[] array. Therefore, this number MUST be less than or
 * equal to MAX_UDP_SOCKETS + the number of TCP sockets defined by the
...
// -- HTTP2 Server options -----------------------------------------------------
    ...
    // Maximum numbers of simultaneous HTTP connections allowed. Each connection
    // consumes 2 bytes of RAM and a TCP socket
    #define MAX_HTTP_CONNECTIONS	(2u)

    // Optional setting to use PIC RAM instead of Ethernet/Wi-Fi RAM for storing
    // HTTP Connection Context variables (HTTP_CONN structure for each HTTP
    // connection). Undefining this macro results in the Ethernet/Wi-Fi RAM
    // being used (minimum PIC RAM usage, lower performance). Defining this
    // macro results in PIC RAM getting used (higher performance, but uses PIC
    // RAM). This option should not be enabled on PIC18 devices. The performance
    // increase of having this option defined is only apparent when the HTTP
    // server is servicing multiple connections simultaneously.
    ...
    #define HTTP_DEFAULT_LEN    (10u)   // For buffer overrun protection. Set to
                                        // longest length of above two strings.

    // Configure MPFS over HTTP updating
    // Comment this line to disable updating via HTTP
    #define HTTP_MPFS_UPLOAD    "mpfsupload"
    //#define HTTP_MPFS_UPLOAD_REQUIRES_AUTH  // Require passwd for MPFS uploads
    ...
    // Define the listening port for the HTTPS server (if STACK_USE_SSL_SERVER
    // is enabled)
    #define HTTPS_PORT              (443u)

    // Define the max data length for reading cookie and GET/POST arguments (bytes)
    #define HTTP_MAX_DATA_LEN	(100u)

    // Define the min number of bytes free in the TX FIFO before executing callbacks
    #define HTTP_MIN_CALLBACK_FREE	(16u)

    //#define STACK_USE_HTTP_APP_RECONFIG
        // Use the AppConfig web page in the Demo App (~2.5kb ROM, ~0b RAM)
    ...
    // Bits in SSL RSA key.  This parameter is used for SSL sever connections
    // only. The only valid value is 512 bits (768 and 1024 bits do not work at
    // this time).  Note, however, that SSL client operations do currently work
    // up to 1024 bit RSA key length.
    #define SSL_RSA_KEY_SIZE		(512ul)

// -- Telnet Options -----------------------------------------------------------

    // Number of simultaneously allowed Telnet sessions. Note that you must have
    // an equal number of TCP_PURPOSE_TELNET type TCP sockets declared in the
    // TCPSocketInitializer[] array above for multiple connections to work. If
    // fewer sockets are available than this definition, then the the lesser of
    // the two quantities will be the actual limit.
    ...
    #define TELNET_PASSWORD		"LSI_B105"
...
// -- SNMP Options -------------------------------------------------------------
    ...
    // This is the maximum length for community string. Application must ensure
    // that this length is observed. SNMP module adds one byte extra after
    // SNMP_COMMUNITY_MAX_LEN for adding '\0' NULL character.
    ...
    // alternate strings are present in external EEPROM or Flash (actual strings
    // are stored in AppConfig.readCommunity[] and AppConfig.writeCommunity[]
    // arrays). These strings are case sensitive.
    ...
    // start development.Specifying more strings than SNMP_MAX_COMMUNITY_SUPPORT
    // will result in the later strings being ignored (but still wasting program
    // memory). Specifying fewer strings is legal, as long as at least one is
    // present.Strings larger than SNMP_COMMUNITY_MAX_LEN bytes will be ignored.
