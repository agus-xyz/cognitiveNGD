 ...
 */
#define STACK_USE_UART			// Application demo using UART for IP address display and stack configuration //DIA 21_6_11
//#define STACK_USE_UART2TCP_BRIDGE	// UART to TCP Bridge application example
//#define STACK_USE_IP_GLEANING
#define STACK_USE_ICMP_SERVER		// Ping query and response capability
#define STACK_USE_ICMP_CLIENT		// Ping transmission capability
//#define STACK_USE_HTTP_SERVER		// Old HTTP server
//#define STACK_USE_HTTP2_SERVER	// New HTTP server with POST, Cookies, Authentication, etc. //DIA 5_8_11, quito el servidor web...
...
//#define STACK_USE_ANNOUNCE		// Microchip Embedded Ethernet Device Discoverer server/client //DIA 5_8_11
//#define STACK_USE_DNS			// Domain Name Service Client for resolving hostname strings to IP addresses
//#define STACK_USE_DNS_SERVER		// Domain Name Service Server for redirection to the local device //DIA 5_8_11
//#define STACK_USE_NBNS		// NetBIOS Name Service Server for repsonding to NBNS hostname broadcast queries //DIA 5_8_11
//#define STACK_USE_REBOOT_SERVER	// Module for resetting this PIC remotely.  Primarily useful for a Bootloader. //DIA 5_8_11
...
//#define STACK_USE_UDP_PERFORMANCE_TEST        // Module for testing UDP TX performance characteristics.  
    // NOTE: Enabling this will cause a huge amount of UDP broadcast packets to
    // flood your network on the discard port.  Use care when enabling this on
    // production networks, especially with VPNs (could tunnel broadcast traffic
    //across a limited bandwidth connection).
...
/* MPFS Configuration
 *   MPFS is automatically included when required for other applications.  If
 *   your custom application requires it otherwise, uncomment the appropriate
 *   selection.
 */
 ...
 *   If html pages are stored in internal program memory, comment both
 *   MPFS_USE_EEPROM and MPFS_USE_SPI_FLASH, then include an MPFS image (.c or
 *   .s file) in the project. If html pages are stored in external memory,
 *   uncomment the appropriate definition.
 *
 *   Supported serial flash parts include the SST25VFxxxB series.
 */
//#define MPFS_USE_EEPROM //FERNANDO
 ...
 *   Number of EEPROM bytes to be reserved before MPFS storage starts. These
 *   bytes host application configurations such as IP Address, MAC Address, and
...
#define MY_DEFAULT_MAC_BYTE1        (0x00)  //Use the default of 0004A3000000
#define MY_DEFAULT_MAC_BYTE2        (0x04)  //if using an ENCX24J600, MRF24WB0M,
#define MY_DEFAULT_MAC_BYTE3        (0xA3)  //or PIC32MX6XX/7XX internal Ethernet
...
#define MY_DEFAULT_GATE_BYTE4       (1ul) //DIA 16_8_11
...
#define MY_DEFAULT_PRIMARY_DNS_BYTE4	(1ul) //DIA 16_8_11
...
// =======================================================================
#define	ETH_CFG_LINK        0	//set to 1 if you need to config the link to
                                //specific following parameters otherwise the
                                //default connection will be attempted depending
                                //on the selected PHY
...
// Size of a RX buffer. should be multiple of 16 this is the size of all receive
// buffers processed by the ETHC. The size should be enough to accomodate any
// network received packet. If the packets are larger, they will have to take
// multiple RX buffers. The current implementation does not handle this situation
// right now and the packet is discarded.
 ...
 *   The following low level modules are automatically enabled based on module
 *   selections above.  If your custom module requires them otherwise, enable
 *   them here.
 */
 ...
 *   TCP needs memory to buffer incoming and outgoing data. The amount and 
 *   medium of storage can be allocated on a per-socket basis using the example
 *   below as a guide.
 */
    // Allocate how much total RAM (in bytes) you want to allocate
    // for use by your TCP TCBs, RX FIFOs, and TX FIFOs.
//    #if defined (TCP_ETH_RAM_SIZE)      //Juan. #define conflict with TCPIP.h
//        #undef TCP_ETH_RAM_SIZE         //Juan
//    #endif                              //Juan
    #define TCP_ETH_RAM_SIZE        (8192ul)

    #if defined (TCP_PIC_RAM_SIZE)      //Juan, idem.
        #undef TCP_PIC_RAM_SIZE
    #endif
    #define TCP_PIC_RAM_SIZE        (0ul)
        
    #if defined (TCP_SPI_RAM_SIZE)      //Juan, idem.
 ...
 *   Define the maximum number of available UDP Sockets, and whether or not to
 *   include a checksum on packets being transmitted.
 */
...
//#define UDP_USE_TX_CHECKSUM   // This slows UDP TX performance by nearly 50%,
                                ...
                                // and incurs virtually no speed pentalty.
