/*******************************************************************************
 ...
 “Company”) for its PIC® Microcontroller is intended and supplied to you, the
 Company’s customer, for use solely and exclusively on Microchip PIC
...
/* Juan - Copiado de demos de USB**********************************************/
#define USB_INITIAL_VBUS_CURRENT 100/2
#define NUM_TPL_ENTRIES 1
/* End Juan********************************************************************/

#define USB_EP0_BUFF_SIZE   8   // Valid Options: 8, 16, 32, or 64 bytes. Using
                                // larger options take more SRAM, but does not
                                // provide much advantage in most types of
				// applications. Exceptions to this, are those
                                // that use EP0 IN or OUT for sending large
...
//#define USB_PING_PONG_MODE USB_PING_PONG__ALL_BUT_EP0	 
    // NOTE: USB_PING_PONG__ALL_BUT_EP0 mode is not supported in PIC18F4550
    // family rev A3 devices
...
//finished processing the control transfer. Therefore, the status stage completes
//as quickly as is physically possible. The USB_ENABLE_STATUS_STAGE_TIMEOUTS
...
//   and some abnormal error occurs, where the host might try to abort the
//   control transfer, before it has sent all of the data it claimed it was
//   going to send.
//
...
#define USB_ENABLE_STATUS_STAGE_TIMEOUTS    //Comment to disable this feature.  
...
//50ms. If the USB_ENABLE_STATUS_STAGE_TIMEOUTS feature is not enabled, then the
//USB_STATUS_STAGE_TIMEOUT parameter is not relevant.

#define USB_STATUS_STAGE_TIMEOUT     (BYTE)45
//Approximate timeout in milliseconds, except when USB_POLLING mode is used, and
//USBDeviceTasks() is called at < 1kHz. In this special case, the timeout (ms)
//becomes approximately:
// (1000*(USB_STATUS_STAGE_TIMEOUT-1))/(USBDeviceTasks() polling frequency in Hz)
//------------------------------------------------------------------------------
...
#define USB_CDC_SUPPORT_ABSTRACT_CONTROL_MANAGEMENT_CAPABILITIES_D1
    //Set_Line_Coding, Set_Control_Line_State, Get_Line_Coding, and Serial_State
    //commands
//------------------------------------------------------------------------------
