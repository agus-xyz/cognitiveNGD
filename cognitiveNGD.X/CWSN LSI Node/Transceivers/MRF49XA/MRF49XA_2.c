/*******************************************************************************
* FileName:	MRF49XA_2.c                   ¡¡¡MODIFIED FROM ORIGINAL STACK!!!
* Dependencies:
...
#include "Transceivers/Transceivers.h"
    ...
    /***************************************************************************
     * WORD MRF49XA_2_getReceiverBW(void)
    ...
    /***************************************************************************
     * void MRF49XA_2_RegisterSet(INPUT WORD setting)
     ...
     **************************************************************************/
    void MRF49XA_2_RegisterSet(WORD setting){
        BYTE oldMRF49XA_2_IE = MRF49XA_2_IE;
        ...
        MRF49XA_2_PHY_CS = 1;
        MRF49XA_2_IE = oldMRF49XA_2_IE;
    }

    /***************************************************************************
     * void MRF49XA_2_StatusRead(void)
     * Overview:        This function read the status register and output the
     *                  status in the global variable MRF49XA_2_Status.
     * PreCondition:    MRF49XA transceiver has been properly initialized
     * Input:           None
     * Output:          None
     * Side Effects:    Global variable MRF49XA_2_Status has been modified
     **************************************************************************/
    void MRF49XA_2_StatusRead(void){
        BYTE preMRF49XA_2_nFSEL = MRF49XA_2_nFSEL;
        ...
        MRF49XA_2_nFSEL = preMRF49XA_2_nFSEL;
    ...
    /***************************************************************************
     * BOOL MRF49XA_2_TxPacket(INPUT BYTE MRF49XA_2_TxPacketLen, INPUT BOOL CCA)
     * Overview:        This function send the packet in the buffer MACTxBuffer
     * PreCondition:    MRF49XA transceiver has been properly initialized
     * Input:           BYTE MRF49XA_2_TxPacketLen    The length of the packet
     *                  to be sent.
...
Start_Transmitting:
                ...
                if( CCA ){
                    MRF49XA_2_RegisterSet(PMCREG | 0x0080);
Start_CCA:
                    DQDOK = 0;
                    for(i = 0; i < CCA_TIMES; i++){
                        MRF49XA_2_StatusRead();
                        if( MRF49XA_2_Status.bits.DQD ){
                            DQDOK++;
                        }
                        Delay10us(1);
                    }
            ...
            MRF49XA_2_RegisterSet(PMCREG);
            MRF49XA_2_RegisterSet(MRF49XA_2_GENCREG | 0x0080);

            // enable transmitter
            MRF49XA_2_RegisterSet(PMCREG | 0x0020);

            DelayMs(1);

            MRF49XA_2_TxPacketPtr = 0;
            synCount = 0;
            ...
            counter = 0;
            while(MRF49XA_2_TxPacketPtr < MRF49XA_2_TxPacketLen + 2 ){   // 2 dummy byte
                if( MRF49XA_2_SPI_SDI == 1 ){
                    if( synCount < 3 ){
                        switch(synCount){
                            ...
                            case 1:
                                ...
                                break;
                            case 2:
                                MRF49XA_2_SPIPut(MRF49XA_2_TxPacketLen);
                                break;
                            default:
                                break;
                        }
                        ...
                        MRF49XA_2_SPIPut(MACTxBuffer[MRF49XA_2_TxPacketPtr]);
                        MRF49XA_2_TxPacketPtr++;
                    }
                    counter = 0;
                }
                else{
                    if( counter++ > 0xFFFE ){
                        ...
                        MRF49XA_2_RegisterSet(MRF49XA_2_GENCREG | 0x0040 );
                        MRF49XA_2_RegisterSet(FIFORSTREG | 0x0002);
                        DelayMs(5);
                        MRF49XA_2_IE = 1;
...
//    }
    ...
    /***************************************************************************
     * Function:    BOOL MiMAC_MRF49XA_2_SetChannel(BYTE channel, BYTE offsetFreq)
     ...
     *      MiMAC_MRF49XA_2_SetChannel(12, 0);
     *      </code>
     * Remarks:     None
     **************************************************************************/
    BOOL MiMAC_MRF49XA_2_SetChannel(INPUT BYTE channel, INPUT BYTE offsetFreq){
        if( channel >= MRF49XA_2_CHANNEL_NUM ){
            return FALSE;
        }

        MRF49XA_2_RegisterSet(0xA000 + MRF49XA_2_FREQ_START + ((WORD)channel * \
                              MRF49XA_2_FREQ_STEP) + offsetFreq);
        DelayMs(20);       // delay 20 ms to stablize the transceiver
        return TRUE;
    }

    /***************************************************************************
     * Function:    BOOL MiMAC_MRF49XA_2_SetPower(BYTE outputPower)
     ...
     *      MiMAC_MRF49XA_2_SetPower(TX_POWER_0_DB);
     *      </code>
     * Remarks:     None
     **************************************************************************/
    BOOL MiMAC_MRF49XA_2_SetPower(INPUT BYTE outputPower){
        if(outputPower > TX_POWER_N_17_5_DB){
            return FALSE;
        }
        MRF49XA_2_RegisterSet(0x9800 | (((WORD)MRF49XA_2_RF_DEV/15 - 1)<<4) | \
                              outputPower);
        return TRUE;
    }

    /***************************************************************************
     * Function:    BOOL MiMAC_MRF49XA_2_Init(MACINIT_PARAM initValue)
     ...
     * Example:
     *      <code>
     *      MiMAC_MRF49XA_2_Init(initParameter);
     *      </code>
     * Remarks:     None
     **************************************************************************/
    BOOL MiMAC_MRF49XA_2_Init(INPUT MACINIT_PARAM initValue){
        BYTE i;
        ...
        MRF49XA_2_RegisterSet(FIFORSTREG);
        MRF49XA_2_RegisterSet(FIFORSTREG | 0x0002); // enable synchron latch
        MRF49XA_2_RegisterSet(MRF49XA_2_GENCREG);
        MRF49XA_2_RegisterSet(AFCCREG);
        MRF49XA_2_RegisterSet(0xA7D0);
        MRF49XA_2_RegisterSet(MRF49XA_2_DRVSREG);
        MRF49XA_2_RegisterSet(PMCREG);
        MRF49XA_2_RegisterSet(MRF49XA_2_RXCREG | MRF49XA_2_getReceiverBW());
        MRF49XA_2_RegisterSet(MRF49XA_2_TXCREG);
        MRF49XA_2_RegisterSet(BBFCREG);
        // antenna tuning on startup
        MRF49XA_2_RegisterSet(PMCREG | 0x0020);     // turn on the TX
        DelayMs(5);         // ant.tuning time (~100us) + Xosc starup time (5ms)
        // end of antenna tuning
        MRF49XA_2_RegisterSet(PMCREG | 0x0080);     // turn off TX, turn on RX
        MRF49XA_2_RegisterSet(MRF49XA_2_GENCREG | 0x0040);    // enable the FIFO
        MRF49XA_2_RegisterSet(FIFORSTREG);
        MRF49XA_2_RegisterSet(FIFORSTREG | 0x0002); // enable synchron latch
        MRF49XA_2_RegisterSet(0x0000);              // read status byte (read ITs)

        return TRUE;
    }

    /***************************************************************************
     * Function:    BOOL MiMAC_MRF49XA_2_SendPacket( MAC_TRANS_PARAM transParam,
     ...
     * Example:
     *      <code>
     *      MiMAC_MRF49XA_2_SendPacket(transParam, MACPayload, MACPayloadLen);
     *      </code>
     * Remarks:     None
     **************************************************************************/
    BOOL MiMAC_MRF49XA_2_SendPacket (INPUT MAC_TRANS_PARAM transParam,
        ...
        return MRF49XA_2_TxPacket(TxIndex, MACInitParams.actionFlags.bits.CCAEnable);
    }

    /***************************************************************************
     * No MCHP doc. available
     **************************************************************************/
    BOOL MiMAC_MRF49XA_2_ReceivedPacket(void){
        BYTE i;
        ...
        for(i = 0; i < BANK_SIZE; i++){
            if( RxPacket[i].flags.bits.Valid ){
                ...
                MRF49XA_2_MACRxPacket.flags.Val = RxPacket[i].Payload[0];
                MRF49XA_2_MACRxPacket.PayloadLen = RxPacket[i].PayloadLen;
                PayloadIndex = 2;

                if( MRF49XA_2_MACRxPacket.flags.bits.destPrsnt ){
                    PayloadIndex += MACInitParams.actionFlags.bits.PAddrLength;
                }

                if( MRF49XA_2_MACRxPacket.flags.bits.sourcePrsnt ){
                    MRF49XA_2_MACRxPacket.SourceAddress = \
                                (BYTE *)&(RxPacket[i].Payload[PayloadIndex]);
                    PayloadIndex += MACInitParams.actionFlags.bits.PAddrLength;
                }
                else {
                    MRF49XA_2_MACRxPacket.SourceAddress = NULL;
                }

                #if defined(ENABLE_SECURITY)
                    if(MRF49XA_2_MACRxPacket.flags.bits.secEn){
                        ...
                        #endif
                        MRF49XA_2_MACRxPacket.PayloadLen -= (PayloadIndex + SEC_MIC_LEN);
                    }
                    else{
                        MRF49XA_2_MACRxPacket.PayloadLen -= PayloadIndex;
                    }
                #else
                    MRF49XA_2_MACRxPacket.PayloadLen -= PayloadIndex;
                #endif

                MRF49XA_2_MACRxPacket.Payload = (BYTE *)&(RxPacket[i].Payload[PayloadIndex]);
                #if !defined(TARGET_SMALL)
                    if(RxPacket[i].flags.bits.RX_RSSI){
                        #if MRF49XA_RSSI_THRESHOLD == RSSI_THRESHOLD_103
                            MRF49XA_2_MACRxPacket.RSSIValue = 1;
                        #elif MRF49XA_RSSI_THRESHOLD == RSSI_THRESHOLD_97
                            MRF49XA_2_MACRxPacket.RSSIValue = 7;
                        #elif MRF49XA_RSSI_THRESHOLD == RSSI_THRESHOLD_91
                            MRF49XA_2_MACRxPacket.RSSIValue = 13;
                        #elif MRF49XA_RSSI_THRESHOLD == RSSI_THRESHOLD_85
                            MRF49XA_2_MACRxPacket.RSSIValue = 19;
                        #elif MRF49XA_RSSI_THRESHOLD == RSSI_THRESHOLD_79
                            MRF49XA_2_MACRxPacket.RSSIValue = 25;
                        #elif MRF49XA_RSSI_THRESHOLD == RSSI_THRESHOLD_73
                            MRF49XA_2_MACRxPacket.RSSIValue = 31;
                        #endif
                    }
                    else{
                        MRF49XA_2_MACRxPacket.RSSIValue = 0;
                    }
                    MRF49XA_2_MACRxPacket.LQIValue = RxPacket[i].flags.bits.DQD;
                #endif
                ReceivedBankIndex = i;
    ...
    /***************************************************************************
     * Function:    MiMAC_MRF49XA_2_DiscardPacket(void)
     ...
     * Example:
     *      <code>
     *      if (TRUE == MiMAC_MRF49XA_2_ReceivedPacket()){
     *          // handle the raw data from RF transceiver
     *          // discard the current packet
     *          MiMAC_MRF49XA_2_DiscardPacket();
     *      }
     *      </code>
     * Remarks:     None
     **************************************************************************/
    void MiMAC_MRF49XA_2_DiscardPacket(void){
        ...
        /***********************************************************************
         * Function:    MiMAC_MRF49XA_2_ChannelAssessment(BYTE AssessmentMode)
         ...
         * Example:
         *      <code>
         *      NL = MiMAC_MRF49XA_2_ChannelAssessment(CHANNEL_ASSESSMENT_CARRIER_SENSE);
         *      </code>
         * Remarks:     None
         **********************************************************************/
        BYTE MiMAC_MRF49XA_2_ChannelAssessment(BYTE AssessmentMode){
            BYTE i;
            BYTE j;
            BYTE k;
            ...
            MRF49XA_2_RegisterSet(PMCREG | 0x0080);    // turn on the receiver
            for(i = 0; i < 6; i++){
                MRF49XA_2_RegisterSet(((MRF49XA_2_RXCREG & 0xFF80) + 5-i) | MRF49XA_2_getReceiverBW());

                count = 0;
                for(j = 0; j < 0xFE; j++){
                    MRF49XA_2_StatusRead();
                    if(AssessmentMode == CHANNEL_ASSESSMENT_CARRIER_SENSE){
                        count += MRF49XA_2_Status.bits.DQD;
                    }
                    else if(AssessmentMode == CHANNEL_ASSESSMENT_ENERGY_DETECT){
                        count += MRF49XA_2_Status.bits.RSSI_ATS;
                    }
                    MRF49XA_2_PHY_CS = 1;
                    for(k = 0; k < 0x1F; k++) {}
                }
                ...
                if(count > k * (7-(MRF49XA_2_getReceiverBW()>>5))){
                    MRF49XA_2_RegisterSet(MRF49XA_2_RXCREG | MRF49XA_2_getReceiverBW());
                    return result[i];
                }
            }
        ...
        /***********************************************************************
         * Function:    MiMAC_MRF49XA_2_PowerState(BYTE PowerState)
         ...
         *      MiMAC_MRF49XA_2_PowerState(POWER_STATE_DEEP_SLEEP);
         *      // Put MCU to sleep
         *      Sleep();
         *      // Wake up the MCU by WDT, external interrupt or any other means
         *      // Wake up the RF transceiver
         *      MiMAC_MRF49XA_2_PowerState(POWER_STATE_OPERATE);
         *      </code>
         * Remarks:     None
         **********************************************************************/
        BOOL MiMAC_MRF49XA_2_PowerState(INPUT BYTE PowerState){
            switch(PowerState){
                case POWER_STATE_DEEP_SLEEP:
                    {
                        MRF49XA_2_RegisterSet(FIFORSTREG);          // turn off FIFO
                        MRF49XA_2_RegisterSet(MRF49XA_2_GENCREG);   // disable FIFO, TX_latch
                        MRF49XA_2_RegisterSet(PMCREG);              // turn off both receiver and transmitter
                        MRF49XA_2_StatusRead();                     // reset all non latched interrupts
                        MRF49XA_2_nFSEL = 1;
                    }
                    break;
                ...
                case POWER_STATE_OPERATE:
                    {
                        ...
                        MRF49XA_2_RegisterSet(PMCREG | 0x0008);     // switch on oscillator
...
IGNORE_HERE:
                    ...
                    MRF49XA_2_IF = 0;
                    MRF49XA_2_RegisterSet(PMCREG);  // turn off the transmitter and receiver
                    MRF49XA_2_RegisterSet(FIFORSTREG);          // reset FIFO
                    MRF49XA_2_RegisterSet(MRF49XA_2_GENCREG);   // disable FIFO, TX_latch
                    MRF49XA_2_RegisterSet(MRF49XA_2_GENCREG | 0x0040);  // enable FIFO
                    MRF49XA_2_RegisterSet(PMCREG | 0x0080);     // turn on receiver
                    MRF49XA_2_RegisterSet(FIFORSTREG | 0x0002); // FIFO synchron latch re-enabled
                    goto RETURN_HERE;
                }
                ...
                while(1){
                    if(MRF49XA_2_FINT == 1){
                        if(bAck){
                            ...
                            RxPacket[BankIndex].Payload[RxPacketPtr++] = MRF49XA_2_SPIGet();
                        }
                        ...
                        if(RxPacketPtr >= PacketLen){ //RxPacket[BankIndex].PayloadLen )
                            ...
                            if(received_crc != calculated_crc){
                                ...
                                MRF49XA_2_RegisterSet(FIFORSTREG | 0x0002); // FIFO synchron latch re-enable
                                ...
                                RxPacket[BankIndex].flags.bits.RX_RSSI = MRF49XA_2_Status.bits.RSSI_ATS;
                            #endif
                            ...
                            #endif
                            {
                                ...
                                if(RxPacket[BankIndex].Payload[0] & DSTPRSNT_MASK){
                                    ...
                                    RxPacket[BankIndex].flags.bits.Valid = TRUE;
                                    #if !defined(TARGET_SMALL)
                                        RxPacket[BankIndex].flags.bits.RSSI = MRF49XA_2_Status.bits.RSSI_ATS;
                                        RxPacket[BankIndex].flags.bits.DQD = MRF49XA_2_Status.bits.DQD;
                                    #endif
                                #endif
                                MRF49XA_2_RegisterSet(FIFORSTREG | 0x0002);
